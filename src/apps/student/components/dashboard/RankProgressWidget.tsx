...
const MAYA_RANKS = {
  NACOM: { name: 'Detective Novato', icon: '🔍', color: 'from-blue-500 to-cyan-500', border: 'border-blue-400', shadow: 'shadow-blue-200' },
  BATAB: { name: 'Sargento', icon: '🏹', color: 'from-green-500 to-emerald-500', border: 'border-green-400', shadow: 'shadow-green-200' },
  HOLCATTE: { name: 'Teniente', icon: '🗡️', color: 'from-purple-500 to-pink-500', border: 'border-purple-400', shadow: 'shadow-purple-200' },
  GUERRERO: { name: 'Capitán', icon: '⚔️', color: 'from-orange-500 to-amber-500', border: 'border-orange-400', shadow: 'shadow-orange-200' },
  MERCENARIO: { name: 'Comisario', icon: '👑', color: 'from-yellow-400 to-amber-400', border: 'border-yellow-400', shadow: 'shadow-yellow-200' },
};
...
export function RankProgressWidget({ data, loading }: RankProgressWidgetProps) {
  ...
  const progressPercent = (data.currentXP / data.nextRankXP) * 100;
  const xpRemaining = data.nextRankXP - data.currentXP;
