...
export const EnhancedStatsGrid: React.FC<EnhancedStatsGridProps & { compact?: boolean }> = ({ stats, loading, error, compact = false }) => {
  ...
  const formatRank = (position: number): string => {
    if (position === 0) return 'Sin ranking';
  ...
  const statCards: StatCard[] = [
    ...
    },
    {
      icon: Flame,
      label: 'Racha Actual',
      value: `${stats.currentStreak} ${getStreakEmoji(stats.currentStreak)}`,
      subtitle: stats.currentStreak > 0 ? 'días consecutivos' : 'Comienza tu racha hoy',
      color: 'text-red-600',
      bgColor: 'bg-red-100',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-900">
              {Math.ceil(stats.casesResolved / 10) * 10}
            </div>
            <div className="text-sm text-blue-600">Casos objetivo</div>
            <div className="text-xs text-blue-500 mt-1">
              {Math.ceil(stats.casesResolved / 10) * 10 - stats.casesResolved} casos restantes
            </div>
          </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-900">
              {Math.ceil(stats.totalXP / 1000) * 1000}
            </div>
            <div className="text-sm text-purple-600">XP objetivo</div>
            <div className="text-xs text-purple-500 mt-1">
              {Math.ceil(stats.totalXP / 1000) * 1000 - stats.totalXP} XP restantes
            </div>
          </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-red-900">
              {Math.max(7, Math.ceil((stats.currentStreak + 1) / 7) * 7)}
            </div>
            <div className="text-sm text-red-600">Días objetivo</div>
            <div className="text-xs text-red-500 mt-1">
              {Math.max(7, Math.ceil((stats.currentStreak + 1) / 7) * 7) - stats.currentStreak} días restantes
            </div>
          </div>
        </div>
      </motion.div>
